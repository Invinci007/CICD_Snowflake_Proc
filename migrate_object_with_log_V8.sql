-- Snowflake Procedure to Migrate Objects between Databases using JavaScript with Logging (V8)
...
-- Usage:
-- CALL MIGRATE_OBJECT_WITH_LOG_V8('SOURCE_DB', 'TARGET_DB', 'SOURCE_SCHEMA', 'TARGET_SCHEMA', 'TABLE', 'MY_TABLE');

CREATE OR REPLACE PROCEDURE MIGRATE_OBJECT_WITH_LOG_V8(
    SOURCE_DATABASE VARCHAR,
    TARGET_DATABASE VARCHAR,
    SOURCE_SCHEMA VARCHAR,
    TARGET_SCHEMA VARCHAR,
...
try {
    ...
    switch (object_type_upper) {
        ...
        case "TASK":
            ...
            // For tasks, only change the task name itself. Use a non-global, case-insensitive
            // regex to replace only the first occurrence of the DB and schema names.

            // 1. Replace the database name (first occurrence only)
            var db_search_regex = new RegExp(`\\b${SOURCE_DATABASE}\\b`, 'i');
            var new_ddl = ddl.replace(db_search_regex, TARGET_DATABASE);

            // 2. Replace the schema name (first occurrence only)
            var schema_search_regex = new RegExp(`\\b${SOURCE_SCHEMA}\\b`, 'i');
